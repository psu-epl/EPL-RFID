...
char PcdAnticoll(uint8_t cascade, uint8_t *pSnr)
{
    ...
    do {
        ...
        ESP_LOGE(READER13, "status1 %x ",status);
    ...
    {
        ESP_LOGE(READER13, "here %x ",status);
        for (i=0; i < 4; i++)
        {
            *(pSnr+i)  = ucComMF522Buf[i];
            ESP_LOGE(READER13, "ID %x ",ucComMF522Buf[i]);
...
uint8_t ReadRawRC(uint8_t Address)
{
    ...
    t.addr = (0x80 | (Address << 1)); //MSB == 1 is for reading from Datasheet section 8.1.2.3.
    t.length = 8;
...
*/
    ...
    spi_transaction_t t;
    memset(&t, 0, sizeof(t)); //Zero out the transaction
    t.addr = Address << 1;        //MSB == 0 is for writing from Datasheet section 8.1.2.3.
    t.tx_buffer = (void *)&value;
